    ...
    try {
      if (mode === 'login') {
        ...
        if (res.success && res.account) {
          const newProf = accountToProfile(res.account, currentProfile);
          onProfileUpdated(newProf);
          setSuccessMessage(`Conta criada com sucesso no Supabase! Bem-vindo(a), ${res.account.name}.`);
          setTimeout(() => {
            onClose();
          }, 1500);
        } else {
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-400">
                {mode === 'login' ? 'Inicie sessão na sua conta' : 'Crie uma nova conta com email'}
              </p>
            </div>
          </div>
